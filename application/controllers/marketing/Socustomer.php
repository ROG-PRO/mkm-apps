...
class Socustomer extends CI_Controller
{
    ...
    */
    ...
    {
        //ambil no po yg diproses
        $id = $this->uri->segment('4');
        //ambil data PO
        $datapo = $this->pocustomer_model->getbyno($id);
        //ambil data part po
        $datapart = $this->pocustomer_model->getdetailpart($id);

        //generate nomor SO berdasarkan data yg di input
        $sono = "SO-" . $datapo->po_proses . "-" . $datapo->po_jenis . "-" . $datapo->customer_cd2 . "-" . $this->newno() . "-" . date('m') . "-" . date('d', strtotime($datapo->po_deliverydate)) . "-" . date('m', strtotime($datapo->po_deliverydate)) . "-" . date('y', strtotime($datapo->po_deliverydate));
        echo $sono;
        //membuat array data untuk insert ke table socustomer
        $data = [
            "so_no" => $sono,
            "so_date" => date('Y-m-d'),
            "so_customerid" => $datapo->customer_cd2,
            "so_pono" => $datapo->po_no,
            "so_project" => $datapo->po_project,
            "so_deliverydate" => $datapo->po_deliverydate,
            "so_proses" => $datapo->po_proses,
            "so_jenis" => $datapo->po_jenis,
            "so_approved" => "0",
            "so_createdby" => $this->session->userdata('user_logged')->username,
            "so_created" => date('Y-m-d H:i:s'),
            "so_flex" => '0'
        ...
        //membuat array untuk insert batch
        $datadetail = array();
        foreach ($datapart as $key => $itmcd) {
            $datadetail[] = array(
                "sod_sono" => $sono,
                "sod_itemcode" => $itmcd->pod_itemcode,
                "sod_itemname" => $datapart[$key]->pod_itemname,
                "sod_model" => $datapart[$key]->pod_model,
                "sod_qty" => $datapart[$key]->pod_qty,
                "sod_unit" => $datapart[$key]->pod_unit,
                "sod_remark" => $datapart[$key]->pod_remark,
                "sod_createdby" => $this->session->userdata('user_logged')->username,
                "sod_created" => date('Y-m-d H:i:s')
            );
        }
        // print_r($datadetail);
        //proses insert batch
        $this->SoCustomer_Model->insertdetail($datadetail);

        //update status PO
        $update = [
            "po_soflex" => "1"
        ];
        $this->pocustomer_model->ubahpocustomer($update, $id);

        $this->session->set_flashdata('success', 'Data PO berhasil dibuat SO!!');
        redirect('marketing/socustomer');
    }
    ...
    public function detail($id = null)
    {
        ...
        $data['title'] = $uri2;
        $data['soid'] = $id;
        $data['so'] = $this->SoCustomer_Model->getbyno($id);
        $this->load->view('templete/header', $data);
        $this->load->view('templete/sidebar');
        $this->load->view('marketing/v_socustomer/detail_so', $data);
        $this->load->view('templete/js');
    }
    ...
    public function cetak($id =  null)
    {
        ...
        $data['soid'] = $id;
        $data['so'] = $this->SoCustomer_Model->getbyno($id);
        $data['so_part'] = $this->SoCustomer_Model->getdetailpart($id);
        //$this->load->view('templete/header', $data);
        //$this->load->view('templete/sidebar');
        $this->load->view('marketing/v_socustomer/cetak_so2', $data);
        //$this->load->view('templete/js');
    }

    public function getdataso()
    {
        $no = $this->input->get('noso');
        $data = $this->SoCustomer_Model->getbyno($no);
        echo json_encode($data);
    }

    public function getdatapartbyso()
    {
        $no = $this->input->get('sono');
        $data = $this->SoCustomer_Model->getdetailpart($no);
        echo json_encode($data);
    }
    ...
    public function approve()
    {
        ...
        $data = [
            "so_approved" => '1',
            "so_approveddate" => date('Y-m-d H:i:s'),
            "so_approvedby" => $this->session->userdata('user_logged')->username

        ];

        $this->SoCustomer_Model->update($id, $data);
        $this->session->set_flashdata('success', 'Approve berhasil!!');
        redirect('marketing/socustomer/d/' . $id);
