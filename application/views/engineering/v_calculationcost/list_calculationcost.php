                        ...
                        <div class="card-body">
                                    ...
                                    <tbody>
                                        ...
                                        ?>
                                            <tr>
                                                ...
                                                <td align="center">
                                                    <!-- add alert select prod type -->
                                                    <?php if ($detailInq['type_prod'] == '') { ?>

                                                        <a onclick="return confirm('Please select prod type first !')" href="#" class='btn btn-secondary btn-sm'><i class='fa fa-calculator' aria-hidden='true'></i></a>
                                                    <?php } else { ?>
                                                ...
                                                <td align="right" style="padding-right: 17px;"><?= number_format(($row->cost_mtl + $row->cost_process + $row->cost_tool + $row->cost_del + $oh_cost), 0, ',', '.'); ?></td>
                            ...
                            <?php foreach ($detailinquiry as $row) {
                                if ($row->eng_approve == 1) {
                                    if ($row->inquiry_status2 == 1) { ?>
                                        <a onclick="cancelConfirm2(' #')" href="#!" class="btn btn-success btn-sm" style="float: right;"><i class="fa fa-times" aria-hidden="true"></i> Cancel Calculation</a>
                                    <?php } else { ?>
...
<div class="modal fade" id="cancelModal2" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel"><i class="fas fa-times" style="color:red"></i> Cancel failed !</h5>
                <button class="close" type="button" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                </button>
            </div>
            <div class="modal-body">Cancel tidak dapat diproses karena status inquiry continue</div>
            <div class="modal-footer">
...
<script>
    ...
    function cancelConfirm2(url) {
        $('#btn-cancel2').attr('href', url);
        $('#cancelModal2').modal();
    }
